...
describe('SmartTransactionsController', () => {
  ...
  describe('onNetworkChange', () => {
    ...
    it('calls "ensureUniqueSmartTransactions" on network change if it is a new chainId', async () => {
      jest
        .spyOn(SmartTransactionsController.prototype, 'checkPoll')
        .mockImplementation(() => ({}));
      const { smartTransactionsState } =
        getDefaultSmartTransactionsControllerState();
      const [smartTransactionForChainId] = createStateAfterPending();
      await withController(
        ...
        async ({ controller, triggerNetworStateChange }) => {
          const initialSmartTransactionsForChainId =
            controller.state.smartTransactionsState.smartTransactions[
              ChainId.mainnet
            ];
          expect(initialSmartTransactionsForChainId).toHaveLength(1);
          expect(initialSmartTransactionsForChainId).toStrictEqual([
            smartTransactionForChainId,
          ]);
          ...
          jest.spyOn(controller, 'isNewSmartTransaction').mockReturnValue(true);
    ...
    it('does not call "ensureUniqueSmartTransactions" on network change for the same chainId', async () => {
      jest
        .spyOn(SmartTransactionsController.prototype, 'checkPoll')
        .mockImplementation(() => ({}));
      const { smartTransactionsState } =
